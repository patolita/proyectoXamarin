...
namespace Arduino.Services
{
    ...
    public class ApiService
    {
        ...
        public async Task<TokenResponse> GetTokenNew(
            string urlBase,
            string username,
            string password)
        {
            try
            {
               ...
               var content = new StringContent(json, Encoding.UTF8, "application/json");
                var response = await client.PostAsync("https://paolita.edlugora96.vercel.app/login", content);
                var resultJSON = await response.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<TokenResponse>(
                    resultJSON);
                return result;
    ...
    User user)
        ...
        string token)
        {
            try
            {
                ...
                //    new AuthenticationHeaderValue(tokenType, accessToken);
                client.BaseAddress = new Uri(urlBase);
                var url = string.Format("{0}{1}{2}{3}", "?function=", "UserMod","&tokenType="+tokenType,"&token="+token);
                var response = await client.PostAsync(url, content);
                ...
                string ImageBase64 = Convert.ToBase64String(user.ImageArray);
                var client = new HttpClient();
                client.BaseAddress = new Uri(urlBase);
                var response = await client.PostAsync("?function=UserMod",
                    new StringContent(string.Format(
                    "imageArray=" + ImageBase64 + "&tokenType=" + tokenType + "&token=" + token),
                    Encoding.UTF8, "application/x-www-form-urlencoded"));
                var resultJSON = await response.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<User>(
                    resultJSON);
